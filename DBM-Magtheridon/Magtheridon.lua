...
local timerHeal				= mod:NewCastTimer(2, 30528, nil, nil, nil, 4, nil)
local timerPhase2			= mod:NewTimer(120, "timerP2", 135566, nil, nil, 1)
local timerBlastNovaCD		= mod:NewCDCountTimer(64, 30616, nil, nil, nil, 2, nil, DBM_COMMON_L.IMPORTANT_ICON)
local timerDebris			= mod:NewNextTimer(15, 36449, nil, nil, nil, 2, nil)--Only happens once per fight, after the phase 3 yell.
...
function mod:SPELL_CAST_START(args)
	...
	elseif args.spellId == 30616 then
		...
		timerBlastNovaCD:Start(45, self.vb.blastNovaCounter)
	end
end
